...
from db_research.service.config import INIT_RECORDS_ALL, INIT_RECORDS_CHUNK, COUNT_OF_SELECTS
...
gen_funcs = {"likes": generate_like, "reviews": generate_review, "bookmarks": generate_bookmark}
...
def fill_mongo():
    ...
    for coll_name, gener_func in gen_funcs.items():
        insert_mongo_gen_objects(coll_name, gener_func, INIT_RECORDS_ALL, INIT_RECORDS_CHUNK)
    print("Filling mongo time ", round(time() - start, 4))


def time_execute_mongo(
    collection,
...
def time_get_likes_user():
    ...
    sum_time = sum(
        (time_execute_mongo(likes_collection, query_mongo, type_f="Find"))
        for _ in range(COUNT_OF_SELECTS)
    )
...
def time_get_bookmarks_user():
    query_mongo = {"user_id": random.choice(user_ids)}
    sum_time = sum(
        (time_execute_mongo(bookmarks_collection, query_mongo, type_f="Find"))
        for _ in range(COUNT_OF_SELECTS)
    )
